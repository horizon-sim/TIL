...
from tkinter import filedialog # 서브모듈이기에 별도로 임포트해야함
...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:/코딩/python test/Python_GUI") 
        # 최초에 경로를 보여줌, r은 뒤에 내용을 탈출문자 무시하고 해준다
...
def del_file():
    ...
    for index in reversed(list_file.curselection()): # reverse는 실제로 바꾸는데 reversed는 바꾼값만 사용 (실제값 영향 없음)
...
def browse_dest_path():
    ...
    txt_dest_path.delete(0, END) # entry기에 END까지인거고 텍스트엿으면 1.0부터 end
...
#########################################################################
    ...
    # 원본 width : 원본 height = 변경 width : 변경 height 비례식으로 간단하게
    # x : y = x' : y'
    # xy' = x'y
    # y' = x'y/x
    # x = width = size[0]
    ...
    # max() : 입력받은 값들 중에서 최대값 반환
    # sum() : 입력받은 모든 값들을 더한값 반환
...
file_frame.pack(fill="x", padx=5, pady=5) # 프레임 간격띄우기
